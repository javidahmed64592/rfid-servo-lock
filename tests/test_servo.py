...
class TestDebugFunction:
    """Unit tests for the debug function."""

    def test_debug_lock_command(
        self, mock_input: MagicMock, mock_gpio: MagicMock, mock_servo_class: tuple[MagicMock, MagicMock]
    ) -> None:
        ...
        mock_servo._lock.assert_called_once()
        mock_servo.cleanup.assert_called_once()
        mock_gpio.cleanup.assert_called_once()

    def test_debug_unlock_command(
        self, mock_input: MagicMock, mock_gpio: MagicMock, mock_servo_class: tuple[MagicMock, MagicMock]
    ) -> None:
        ...
        mock_servo._unlock.assert_called_once()
        mock_servo.cleanup.assert_called_once()
        mock_gpio.cleanup.assert_called_once()

    def test_debug_toggle_command(
        self, mock_input: MagicMock, mock_gpio: MagicMock, mock_servo_class: tuple[MagicMock, MagicMock]
    ) -> None:
    ...
    def test_debug_invalid_command(
        self,
        mock_input: MagicMock,
        mock_gpio: MagicMock,
        mock_servo_class: tuple[MagicMock, MagicMock],
        ...
        assert "Invalid command. Use: lock, unlock, toggle, or quit" in caplog.text
        mock_servo.cleanup.assert_called_once()
        mock_gpio.cleanup.assert_called_once()

    @pytest.mark.parametrize("quit_cmd", ["quit", "q", "exit"])
    def test_debug_quit_variations(
        self, quit_cmd: str, mock_input: MagicMock, mock_gpio: MagicMock, mock_servo_class: tuple[MagicMock, MagicMock]
    ) -> None:
    ...
    def test_debug_keyboard_interrupt(
        self,
        mock_input: MagicMock,
        mock_gpio: MagicMock,
        mock_servo_class: tuple[MagicMock, MagicMock],
